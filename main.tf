...
module "security_group" {
  ...
  ec2_jenkins_name          = "SG for EC2 for enabling port 8080"
}
...
module "jenkins" { 
  ...
  sg_enable_8080           = module.security_group.sg_jenkins_id
  //ec2_sg_name_for_python_api     = module.security_group.sg_ec2_for_python_api
  enable_public_ip_address = true
  user_data_install_jenkins = templatefile("./template/jenkins-script.sh", {})
}
...
module "lb_target_group" {
  ...
  vpc_id                   = module.networking.dev_proj_1_vpc_id
  ec2_instance_id          = module.jenkins.dev_proj_1_ec2_instance_id
}
